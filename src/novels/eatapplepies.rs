...
impl NovelProvider for EatApplePies
{
  ...
  fn get_chapter(&self, html: & str) -> Option<String> {
    ...
    match m.get("title") {
      ...
      Some(d) => {
        let re = regex::Regex::new(r".+Chapter\s0*(\d+)[\s:–-]+(.+)")
          .unwrap();
        let new_data =
          format!("{}\n\n",re.replace(d, "Chapter $1: $2").to_string());
    ...
    match m.get("body") {
      None => return None,
      Some(body) => Some(data.push_str(body))
    };
...
mod tests {
  ...
  "#;
    ...
    let expected =
      "Chapter 1: Prologue\n\n<p>When I opened my eyes, I was inside a novel.</p><p>[The Birth of a Hero].</p><p>[The Birth of a Hero] was a novel focused on the adventures of the main character, Choi Han, a high school boy who was transported to a different dimension from Earth, along with the birth of the numerous heroes of the continent.</p>";

    let received = guinea_pig.get_chapter(&html)
      .expect("Should not have any errors in the parsing!");

    // let _ = std::fs::write("ohno.txt", received);
